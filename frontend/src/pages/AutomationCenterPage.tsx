...
import type { AIStatus, AutomationStatus, PythonEngineStatus } from "../lib/types";
...
export function AutomationCenterPage() {
  ...
  async function load() {
    const [automationData, aiData, pythonData] = await Promise.all([api.automationStatus(), api.aiStatus(), api.pythonEngineStatus()]);
  ...
  return (
    <div className="space-y-6">
      ...
      {pythonStatus ? (
        <section className={`rounded-lg border p-5 shadow-glow ${pythonStatus.connected ? "border-mint/25 bg-mint/10" : "border-caution/25 bg-caution/10"}`}>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-mint">Professional analysis engine</p>
              <h3 className="mt-1 text-lg font-semibold text-stone-50">Python Engine</h3>
              <p className="mt-1 text-sm text-stone-400">
                Technical indicators, multi-timeframe analysis, risk checks, backtests, news scoring, and portfolio analytics.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <SafetyBadge label={pythonStatus.connected ? "PYTHON ENGINE ACTIVE" : "TYPESCRIPT FALLBACK"} tone={pythonStatus.connected ? "green" : "amber"} />
              {!pythonStatus.connected && pythonStatus.enabled ? <SafetyBadge label="ANALYSIS DEGRADED" tone="red" /> : null}
              <SafetyBadge label="PAPER TRADING ONLY" tone="purple" />
            </div>
          </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <StatCard
              label="Connection"
              value={pythonStatus.connected ? "Connected" : pythonStatus.enabled ? "Unavailable" : "Disabled"}
              detail={pythonStatus.fallbackActive ? "Fallback active" : "Python analysis live"}
              icon={Cpu}
              tone={pythonStatus.connected ? "green" : "amber"}
            />
            <StatCard label="Last analysis" value={pythonStatus.lastAnalysisAt ? dateShort(pythonStatus.lastAnalysisAt) : "None yet"} icon={Clock3} tone="neutral" />
            <StatCard label="Indicators" value={pythonStatus.engines.indicators} icon={Gauge} tone={pythonStatus.engines.indicators === "active" ? "green" : "amber"} />
            <StatCard label="Backtesting" value={pythonStatus.engines.backtesting} icon={Activity} tone={pythonStatus.engines.backtesting === "active" ? "green" : "amber"} />
            <StatCard label="Risk engine" value={pythonStatus.engines.risk} icon={ShieldOff} tone={pythonStatus.engines.risk === "active" ? "green" : "amber"} />
          </div>

          {pythonStatus.lastError ? (
            <div className="mt-4 rounded-lg border border-caution/25 bg-black/20 p-3 text-sm text-amber-100">
              Python engine message: {pythonStatus.lastError}
            </div>
          ) : null}
        </section>
      ) : null}
