...
import { getPythonEngineStatus } from "../services/pythonEngineClient.js";
...
apiRouter.get("/system/status", async (_req, res, next) => {
  try {
    ...
    const [aiStatus, automationStatus, pythonEngine, lastStockScan, lastCryptoScan, lastResearch] = await Promise.all([
      getAIStatus(),
      getAutomationStatus(),
...
apiRouter.get("/python-engine/status", async (_req, res, next) => {
  try {
    res.json(await getPythonEngineStatus());
  } catch (error) {
    next(error);
  }
});
