...
export const PAPER_ACCOUNT_SYSTEM_EMAIL = "demo@tradepilot.local";
...
async function ensurePaperAccount(client: PrismaLike = prisma) {
  ...
  const userId = context?.userId ?? (await client.user.upsert({
    where: { email: PAPER_ACCOUNT_SYSTEM_EMAIL },
    update: {},
    create: { name: "TradePilot System", email: PAPER_ACCOUNT_SYSTEM_EMAIL, role: "SYSTEM", realTradingEnabled: false }
  })).id;
...
async function syncPaperTradeToPosition(client: PrismaLike, trade: PaperTrade, accountId: string) {
  ...
  const initialPositionValue = money(trade.entryPrice * trade.quantity + trade.entryFee);
  const positionValue = money((open ? trade.currentPrice : exitPrice ?? trade.currentPrice) * trade.quantity);
  const unrealizedPnL = open ? money((trade.currentPrice - trade.entryPrice) * trade.quantity - trade.entryFee) : 0;
  const realizedPnL = open ? 0 : money(trade.profitLoss);
  const pnlPercent = pct(((open ? unrealizedPnL : realizedPnL) / Math.max(0.01, initialPositionValue)) * 100);
...
export async function reconcilePaperAccount(client: PrismaLike = prisma, options: { createSnapshot?: boolean } = {}) {
  ...
  const updated = await client.paperAccount.update({
    ...
    data: {
      ...
      totalFeesSimulated: money(executionTotals._sum.fee ?? 0),
      totalSlippageSimulated: money(executionTotals._sum.slippageAmount ?? 0),
...
export async function registerPaperTradeOpened(client: PrismaLike, trade: PaperTrade) {
  ...
  await client.paperTradeEvent.create({
    data: {
      ...
      message: `${label} - ${strategyName}. Professional simulated fill; fee ${PAPER_ACCOUNT_CURRENCY} ${trade.entryFee.toFixed(2)}, slippage ${PAPER_ACCOUNT_CURRENCY} ${trade.entrySlippage.toFixed(2)}.`
    }
  });
...
export async function registerPaperTradeClosed(client: PrismaLike, trade: PaperTrade, reason = "manual_close") {
  ...
  const strategyName = position?.strategyName ?? (await tradeStrategyName(client, trade));
  const pnl = money(trade.profitLoss);
  const pnlPercent = pct(trade.profitLossPercent);
  ...
  await client.paperTradeEvent.create({
    data: {
      ...
      strategyName,
      candleTime: closedAt,
      message: `SELL @ ${exitPrice.toFixed(2)} - Net P/L ${pnl >= 0 ? "+" : ""}${pnl.toFixed(2)} after ${PAPER_ACCOUNT_CURRENCY} ${(trade.entryFee + trade.exitFee).toFixed(2)} simulated fees. Reason: ${reason}.`
    }
  });
