...
async function closeIfNeeded(trade: Awaited<ReturnType<typeof prisma.paperTrade.findMany>>[number]) {
  ...
  if (currentPrice <= trade.stopLoss) {
    const closed = await closePaperTrade(trade.id, currentPrice, "Stop-loss hit");
  ...
  if (assetType === "stock") {
    ...
    if (ageDays >= 3 && dailyEma20 > 0 && latestDailyClose < dailyEma20) {
      const closed = await closePaperTrade(trade.id, currentPrice, "Trend exit below EMA20");
      return { updated: true, closed: true, reason: `trend-exit:${closed.status}` };
    }
    if (ageDays >= 21) {
      const closed = await closePaperTrade(trade.id, currentPrice, "Maximum holding period exit");
      return { updated: true, closed: true, reason: `time-exit:${closed.status}` };
    }
  }

  if (currentPrice >= trade.takeProfit) {
    const closed = await closePaperTrade(trade.id, currentPrice, "Take-profit hit");
    await prisma.paperTradeEvent.create({
      data: {
        assetType,
        paperTradeId: trade.id,
  ...
  return runJob("paperTradeUpdateJob", async () => {
    ...
    for (const market of automationMarkets) {
      ...
      for (const signal of candidates) {
        ...
        if (postPlanRisk.blocked || plan.status === "Watchlist Only") continue;
        ...
        await paperTradingAgent({ assetType, ticker: stock.ticker, action: "Opened paper trade", reason: `Passed risk guardrails with ${stock.quoteSource}.` }, "paperTradeUpdateJob");
        await prisma.paperTradeEvent.create({
          data: {
            assetType,
            paperTradeId: trade.id,
            ...
            profitLoss: 0,
            message: `${trade.ticker} paper trade opened from fresh ${stock.quoteSource} data.`
          }
        });
        await createAlert({ assetType, ticker: trade.ticker, alertType: "paper trade opened", severity: "Info", message: `${trade.ticker} automatic paper trade opened.` });
        opened += 1;
      }
    }

    await reconcilePaperAccount(undefined, { createSnapshot: opened > 0 });
    return `Live update complete: ${updatedCount} updated, ${closedCount} closed, ${opened} opened, ${blockedUpdates + blockedEntries} blocked by safeguards.`;
  });
}
