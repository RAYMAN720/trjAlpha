...
import { getTradingControl } from "../professional/tradingControlService.js";
...
export async function advancedRiskApproval(input: {
  ...
  const [status, control] = await Promise.all([
    getAdvancedRiskStatus(input.assetType),
    getTradingControl()
  ]);
  ...
  const controlBlocked = !control.newEntriesEnabled || control.emergencyHalt;
  if (controlBlocked) warnings.push(`Professional safety halt: ${control.reason ?? "new paper entries disabled"}`);
  const blocked = status.tradePaused || capacityBlocked || controlBlocked;
  return {
    approved: !blocked,
    blocked,
    status,
